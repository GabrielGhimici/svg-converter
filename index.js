...
function init() {
  ...
  if (args.length > 2) {
    ...
    if (argsChunks.length < 2) {
      console.log('Please provide dirPath argument...');
      process.exit(1);
    } else {
      if (argsChunks[0] != "dirPath") {
          console.log('Please provide dirPath argument...');
          process.exit(1);
      } else {
    ...
    console.log('Please provide file path in format dirPath="Path to file"...');
    process.exit(1);
  }
}
...
function findPath(data) {
  // Verify if I have a svg
  if (!data.svg) return null;

  /* Treat case of svg which contains only 1 or 2 path node(s). Example: 
  * <svg>
  *   <path></path>
  *   <path></path>
  * </svg
  */
  if (data.svg.path) {
    if (data.svg.path instanceof Array) {
      if (data.svg.path.length == 2) {
        return data.svg.path.find(el => el._attributes.d !== "M0 0h24v24H0z") || null;
      } else {
        return data.svg.path[0];
      }
    } else {
      return data.svg.path
    }
  }

  /* Treat case of svg generated by Sketch, where path node(s) needed is in defs and defs exist. Example: 
  * <svg>
  *   <defs>
  *     <path></path>
  *   </defs>
